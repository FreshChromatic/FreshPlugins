...
final class SeededWorldGenRegion extends WorldGenRegion {
    ...
    static {
        try {
            ...
            throw new ExceptionInInitializerError(
                "SeededWorldGenRegion: WorldGenRegion field layout changed Ã¢â‚¬â€ " + e.getMessage());
        }
    }
    ...
    SeededWorldGenRegion(
        ...
        // Surface's internally-created Blender scans outside its radius-one cache and only needs
        // to observe that an absent boundary chunk has no blending data. Avoid generating a biome
        // palette under ConcurrentHashMap.computeIfAbsent, which serialises concurrent workers.
     ...
     * throws {@link ReportedException} when it cannot satisfy the request Ã¢â‚¬â€ it never returns
    ...
    public boolean isOldChunkAround(ChunkPos pos, int radius) {
        // WorldGenRegion normally delegates this query to IOWorker's region-file cache. That is
        // correct for the live chunk pipeline, but regeneration operates on a private snapshot:
        // the disk cache can lag our direct writes and does not describe which original chunks
        // are present in this synthetic region. Blender.of() must use the same snapshot that its
        // subsequent getChunk() calls will inspect.
        int minX = pos.x - radius;
        int maxX = pos.x + radius;
        int minZ = pos.z - radius;
        int maxZ = pos.z + radius;
        for (Map.Entry<ChunkPos, ProtoChunk> entry : this.globalChunks.entrySet()) {
            ChunkPos candidate = entry.getKey();
            if (candidate.x >= minX && candidate.x <= maxX
                && candidate.z >= minZ && candidate.z <= maxZ
                && entry.getValue().isOldNoiseGeneration()) {
                return true;
            }
        }
        return false;
    ...
    public Holder<Biome> getUncachedNoiseBiome(int quartX, int quartY, int quartZ) {
        ProtoChunk chunk = this.globalChunks.get(new ChunkPos(quartX >> 2, quartZ >> 2));
        if (chunk != null && chunk.getPersistedStatus().isOrAfter(ChunkStatus.BIOMES)) {
            // Existing chunks keep their saved biome cells during regeneration. Surface rules and
            // decoration must observe those same cells; falling straight through to the current
            // BiomeSource can otherwise retain an old desert-pyramid StructureStart while laying
            // a newly-resolved plains/grass surface over it.
            return chunk.getNoiseBiome(quartX & 3, quartY, quartZ & 3);
        }
