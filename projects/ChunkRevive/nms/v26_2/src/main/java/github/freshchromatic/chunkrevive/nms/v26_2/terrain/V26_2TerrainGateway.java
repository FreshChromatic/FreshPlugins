...
public final class V26_2TerrainGateway implements TerrainGateway {
    @Override
    public void initialize(JavaPlugin plugin) {
        V26_2TerrainEngine.init(plugin);
    }

    @Override
    public void configure(TerrainSettings settings) {
        V26_2TerrainEngine.setEntityExemptions(settings.entityExemptions());
        V26_2TerrainEngine.setThreadPoolConfig(settings.threadPool());
        V26_2TerrainEngine.setMemorySafetyConfig(settings.memorySafety());
        V26_2TerrainEngine.setContextRadius(settings.contextRadius());
        V26_2TerrainEngine.setApplyBatchSize(settings.applyBatchSize());
        V26_2TerrainEngine.setDebugLogging(settings.debugLogging());
    }
    ...
    public CompletableFuture<Void> generate(World world, Collection<ChunkCoordinate> chunks, long seed,
        ...
        return V26_2TerrainEngine.generate(
            world, positions, seed, progress, stage, cancelled, extraContextBounds);
    }

    @Override
    public void beginBulkSession(int targetCount) {
        V26_2TerrainEngine.beginBulkSession(targetCount);
    }

    @Override
    public void endBulkSession() {
        V26_2TerrainEngine.endBulkSession();
    }

    @Override
    public int activeGenerationThreads() {
        return V26_2TerrainEngine.getActiveGenerationThreads();
    }

    @Override
    public int resolvedParallelism() {
        return V26_2TerrainEngine.getResolvedParallelism();
    }

    @Override
    public void shutdown() {
        V26_2TerrainEngine.shutdownPool();
    }
}
